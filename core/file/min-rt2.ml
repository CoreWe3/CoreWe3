...
(* 飛ばした光が衝突した物体面番号の配列 *)
(* 物体面番号は オブジェクト番号 lsl 2 + (solverの返り値) *)
...
let rec trace_reflections index diffuse hilight_scale dirvec =
  ...
  if index >= 0 then (
    ...
    if judge_intersection_fast dvec then
      let surface_id = intersected_object_id.(0) lsl 2 + intsec_rectside.(0) in
...
let rec trace_ray nref energy dirvec pixel dist =
  if nref <= 4 then (
    ...
    (* オブジェクトにぶつかった場合 *)
      ...
      (* pixel tupleに情報を格納する *)
      surface_ids.(nref) <- obj_id lsl 2 + intsec_rectside.(0);
...
let rec write_ppm_header _ =
  (
    ...
    print_char 10;
    print_char (48 + 1); (* width fixed to 128px *)
    print_char (48 + 2);
    print_char (48 + 8);
    print_char 32;
    print_char (48 + 1); (* height fixed to 128px *)
    print_char (48 + 2);
    print_char (48 + 8);
    print_char 10;
    print_char (48 + 2);
...
let rec scan_line y prev cur next group_id = (
  ...
  if y < image_size.(1) then (
    ...
    scan_line (y + 1) cur next prev (add_mod5 group_id 2)) else ()
)
in
...
(
 image_size.(0) <- size_x;
 image_size.(1) <- size_y;
 image_center.(0) <- size_x lsr 1;
 image_center.(1) <- size_y lsr 1;
